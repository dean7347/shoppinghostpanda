...
import {useGetAdminPandaSettlementCompleteList, useGetAdminPandaSettlementList} from "./adminPageHooks";
...
const AdminPandaTable = ({selectedMode}) => {
    ...
    const pageChangeAction = useCallback((page) => {
        setPage(page)
        if (selectedMode === '정산필요') {
            refetchSettlement()
        }
    ...
    const confirmSelected = useCallback((event) => {
